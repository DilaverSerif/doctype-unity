...
namespace litehtml
{
    ...
    class render_item : public std::enable_shared_from_this<render_item>
    {
      ...
      public:
        // --- LiteHtmlUnity: retained display list bookkeeping (experiment E2)
        //
        // Lives on the node rather than in a side map because the draw
        // traversal already has the node in cache when it needs these; an
        // unordered_map lookup per child per pass costs more than the emission
        // the cache is trying to avoid.
        //
        //   dc_slot        one entry index per cacheable pass (block / floats /
        //                  inlines). 0 = never visited, 1 = visited once but no
        //                  entry yet, >=2 = index into the host's entry table.
        //   dc_geom        last geometry snapshot taken by the host's post-layout
        //                  diff walk, in the order the walk writes it.
        //   dc_dirty_epoch equals the host's current dirty epoch while this
        //                  subtree's cached output is known to be stale. The
        //                  host bumps its epoch after a full rebuild, which
        //                  makes every node clean again in O(1).
        uint32_t dc_slot[3]      = {0, 0, 0};
        float    dc_geom[14]     = {};
        uint32_t dc_child_count  = 0;
        uint32_t dc_dirty_epoch  = 0;
        bool     dc_geom_valid   = false;
        // Set by the host when a pseudo-class restyled this element: the whole
