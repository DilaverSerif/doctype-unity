...
namespace litehtml
{
    ...
    class document : public std::enable_shared_from_this<document>
    {
      public:
        ...
        // --- LiteHtmlUnity: retained display list hooks (experiment E2) -----
        //
        // A host that caches the output of draw() per subtree needs two things
        // litehtml does not otherwise expose: a chance to skip re-walking a
        // subtree whose output it already has, and a notification when a
        // pseudo-class change (:hover, :active) restyles an element.
        //
        // Deliberately a plain function-pointer table rather than virtuals on
        // document_container: draw_children calls open() once per visible child
        // per pass -- ~3000 times for a 150-row page -- and a virtual dispatch
        // that only ever returns "no" would show up in the numbers of a build
        // with caching switched off, which is exactly the A/B comparison this
        // has to stay honest about. A null m_draw_cache is one predictable
        // branch.
        struct draw_cache_hooks
        {
            void* ctx = nullptr;

            // Called before a child subtree is drawn. Return:
            //   0 -- draw it normally, do not capture,
            //   1 -- already served from cache, skip the subtree entirely,
            //   2 -- draw it normally and capture it (close() will follow).
            int (*open)(void* ctx, render_item* ri, int flag, float x, float y) = nullptr;

            // Closes the capture opened by a return value of 2.
        ...
        const draw_cache_hooks* draw_cache() const { return m_draw_cache; }
      ...
      private:
        ...
        const draw_cache_hooks*                m_draw_cache = nullptr;
