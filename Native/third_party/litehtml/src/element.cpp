...
#define LITEHTML_RETURN_FUNC(ret)                                                                                      \
    ...
    bool element::find_styles_changes(const std::function<void(const position&)>& redraw_box)
    {
        ...
        if(requires_styles_update())
        {
            ...
            // Tell the host before it gets a chance to replay a cached run with
            // the old :hover colours in it.
            if(auto doc = get_document())
            {
                if(const auto* dcache = doc->draw_cache())
                {
                    if(dcache->styles_changed)
                    {
                        dcache->styles_changed(dcache->ctx, this);
                    }
                }
            }
