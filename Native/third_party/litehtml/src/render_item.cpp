                                          ...
                                          int zindex)
{
    ...
    // LiteHtmlUnity (experiment E2): give the host a chance to replay a cached
    // quad run for this child instead of walking it again. The subtree a single
    // loop iteration draws is contiguous in the container's output -- litehtml
    // finishes one child, including its recursion, before starting the next --
    // which is what makes splicing a cached run sound.
    //
    // draw_positioned is deliberately left uncached: the same child is visited
    // once per distinct z-index in that pass, so a per-(node, pass) entry would
    // thrash rather than hit.
    const document::draw_cache_hooks* dcache = flag == draw_positioned ? nullptr : doc->draw_cache();
    ...
    for(const auto& el : m_children)
    {
        if(el->is_visible())
        {
            ...
            if(dcache)
            {
                dc_state = dcache->open(dcache->ctx, el.get(), static_cast<int>(flag), static_cast<float>(pos.x),
