                                                ...
                                                draw_flag flag, int zindex)
{
    ...
    // LiteHtmlUnity (experiment E2): same retained-display-list hook as the
    // generic render_item::draw_children. A cell's own draw plus its recursion
    // is contiguous in the container's output, so it is a cacheable unit; the
    // row backgrounds and captions above are left out because they are a
    // handful of quads either way.
    const document::draw_cache_hooks* dcache =
        flag == draw_positioned ? nullptr : src_el()->get_document()->draw_cache();
    ...
    for(int row = 0; row < m_grid->rows_count(); row++)
    {
        ...
        for(int col = 0; col < m_grid->cols_count(); col++)
        {
            ...
            if(cell->el)
            {
                ...
                if(dcache)
                {
                    dc_state = dcache->open(dcache->ctx, cell->el.get(), static_cast<int>(flag),
