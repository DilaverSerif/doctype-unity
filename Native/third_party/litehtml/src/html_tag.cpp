...
namespace litehtml
{
    ...
    void litehtml::html_tag::compute_styles(bool recursive)
    {
        ...
        if(style)
        {
            // LHU PATCH (experiment E6): reuse the parse of an identical style="..." string
            // instead of re-tokenizing it for every element that carries it. See
            // document_container::cached_inline_style() for why this is a pure memo.
            //
            // combine() is exactly equivalent to add()ing the declarations one by one into
            // m_style: a declaration's parse never reads the map it is written into (style.cpp
            // has no get_property/remove_property callers), and add_parsed_property's only
            // rule is "a write is dropped iff the current value is !important and the write is
            // not", which composes -- folding the block into an empty map first and then
            // applying the folded result gives the same value for every property name.
            //
            // Returns nullptr when the cache is disabled, and then this is litehtml's
            // original single line.
            if(const litehtml::style* cached = doc->container()->cached_inline_style(style))
            {
                m_style.combine(*cached);
            } else
            {
