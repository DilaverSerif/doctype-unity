...
namespace Doctype.Tests
{
    ...
    /// </remarks>
    public class HtmlRuntimeCapture
    {
        [UnityTest]
        public IEnumerator CaptureRunningDemo()
        {
            ...
            foreach (HtmlDemoController.Page page in
                     System.Enum.GetValues(typeof(HtmlDemoController.Page)))
            {
                ...
                Debug.Log($"[Doctype] captured {page}: {view.QuadCount} quads, " +
                          $"{view.TotalMs:0.00} ms rebuild -> {path}");
            }

            // A second frame of the animated page, to show it actually moves.
            controller.GoTo(HtmlDemoController.Page.Animation);
            ...
            Capture(view, Path.Combine(dir, "demo_runtime.png"), HtmlDemoController.Page.Overview, controller);
            yield return null;
            Object.DestroyImmediate(canvasGo);
            Object.DestroyImmediate(camera);
        }

        private static void Capture(HtmlView view, string path,
                                    HtmlDemoController.Page? goTo = null,
                                    HtmlDemoController controller = null)
        {
