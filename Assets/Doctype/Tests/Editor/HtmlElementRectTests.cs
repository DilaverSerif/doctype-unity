...
{
    /// <summary>
    /// Covers <see cref="HtmlDocument.TryGetElementRect"/>, which exists so a
    ...
    /// </remarks>
    public class HtmlElementRectTests
    {
        ...
        static HtmlDocument Load(string html, float viewport = 1000f)
        {
        ...
        public void RectWidthFollowsContentWhileTheDocumentDoesNot()
        {
            ...
            using HtmlDocument shortText = Load(Badge("12"));
            Assert.IsTrue(shortText.TryGetElementRect("#hud", out Rect small));

            using HtmlDocument longText = Load(Badge("123456"));
        ...
        public void RectIsThePaintedBoxNotTheContentBox()
        {
            ...
            using HtmlDocument doc = Load(Badge("12"));
        ...
        {
            using HtmlDocument doc = Load(Grid(), 400f);
        ...
        static void NativeArrayCount(HtmlDocument doc, out int quads)
        {
            doc.Record(out HtmlFrame frame);
