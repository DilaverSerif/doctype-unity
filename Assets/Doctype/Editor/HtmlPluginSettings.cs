...
namespace Doctype.EditorTools
{
    ...
    /// </para>
    /// </remarks>
    public sealed class HtmlPluginSettings : AssetPostprocessor
    {
        const string PluginRoot = "Assets/Doctype/Plugins/";
        ...
        [MenuItem("Tools/Doctype/Fix native plugin import settings")]
        public static void ConfigureAll()
        {
            int fixedUp = 0;

            foreach (string guid in AssetDatabase.FindAssets("", new[] { "Assets/Doctype/Plugins" }))
            {
            ...
            Debug.Log($"[Doctype] configured {fixedUp} native plugin(s)");
        }
        ...
        [MenuItem("Tools/Doctype/Ensure shaders ship with the player")]
        public static void EnsureShadersIncluded()
        {
            ...
            foreach (string name in RuntimeShaders)
            {
                ...
                if (shader == null)
                {
                    Debug.LogError($"[Doctype] shader '{name}' is missing from the project entirely.");
                    continue;
                }
                ...
                Debug.Log($"[Doctype] added '{name}' to Always Included Shaders");
            }
