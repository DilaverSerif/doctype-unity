...
namespace LiteHtmlUnity.Tests
{
    ...
    /// </summary>
    public class LiteHtmlImageTests
    {
        ...
        Texture2D _readback;
        LiteHtmlRenderer _renderer;

        [SetUp]
        public void SetUp()
        {
            _host = new GameObject("Resources");
            _resources = _host.AddComponent<LiteHtmlResources>();
        ...
        LiteHtmlDocument Load(string html)
        {
        ...
        {
            using LiteHtmlDocument doc = Load(
        ...
        public void RecordingEmitsAnImageQuadWithAtlasUvs()
        {
            ...
            {
                if (quads[i].Type != LiteHtmlQuadType.Image)
                {
                    continue;
                }
