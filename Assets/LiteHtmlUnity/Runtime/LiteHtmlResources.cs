...
namespace LiteHtmlUnity
{
    ...
    /// </para>
    /// </remarks>
    [AddComponentMenu("LiteHtml/LiteHtml Resources")]
    public sealed class LiteHtmlResources : MonoBehaviour, ILiteHtmlResourceProvider
    {
        [Serializable]
        ...
        private void Repack()
        {
            ...
            foreach (KeyValuePair<string, Texture2D> pair in _sources)
            {
                ...
                {
                    Debug.LogError($"[LiteHtml] '{pair.Key}' ({pair.Value.name}) needs Read/Write enabled in its " +
            ...
            if (_atlas == null)
            {
                _atlas = new Texture2D(1, 1, TextureFormat.RGBA32, false) { name = "LiteHtml image atlas" };
            }

            Rect[] rects = _atlas.PackTextures(textures.ToArray(), _padding, _maxAtlasSize, false);
            if (rects == null)
            {
                Debug.LogError($"[LiteHtml] could not pack {textures.Count} image(s) into a {_maxAtlasSize}px " +
