...
```
LiteHtmlView.LoadHtml(str)  ──► document::createFromString()      [C++]
           ...
           │
   LiteHtmlMeshBuilder → tek dinamik Mesh (8 UV kanalı = quad parametreleri)
           │
   LiteHtmlRenderer → CommandBuffer → RenderTexture
                      shader: LiteHtmlUnity/Quad
```
...
`Assets/LiteHtmlUnity/Plugins/macOS/` altına koyar. Tek gereksinim Xcode —
CMake gerekmiyor.
...
derleyip Xcode projesine ekleyin. `LiteHtmlNative.Lib` iOS'ta zaten
...
```
RawImage + LiteHtmlRawImage + LiteHtmlView
```
...
```bash
Unity -batchmode -projectPath . -executeMethod LiteHtmlUnity.EditorTools.LiteHtmlDemoSceneBuilder.Build -quit
```

`Assets/LiteHtmlUnity/Samples/LiteHtmlDemo.unity` üretir: Camera + EventSystem
(yeni Input System modülüyle) + Canvas + RawImage üzerinde `LiteHtmlView` +
`LiteHtmlRawImage` + `LiteHtmlDemoController`. Sahne elle değil **script ile**
...
`LiteHtmlRenderTests` gerçek bir `RenderTexture`'a çizip `ReadPixels` ile geri
...
16 test. EditMode testleri `LiteHtmlDocument`/`LiteHtmlRenderer`'ı doğrudan
sürüyor; PlayMode testleri ise `LiteHtmlView`'ın **MonoBehaviour yaşam
...
Demoya özel testler (`LiteHtmlDemoTests`): menüyle gezinme, **nav bar'a gerçek
...
`LiteHtmlRuntimeCapture` her sayfayı ayrı PNG olarak yazar
...
- **Görseller** boru hattı bağlandı (`ILiteHtmlResourceProvider`) ama hazır bir
