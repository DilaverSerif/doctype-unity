...
namespace LiteHtmlUnity.Samples
{
    ...
    /// </para>
    /// </remarks>
    [AddComponentMenu("LiteHtml/Samples/LiteHtml Benchmark")]
    public class LiteHtmlBenchmark : MonoBehaviour
    {
        ...
        LiteHtmlBenchmarkLoad _load;
        LiteHtmlView[] _panels;
        LiteHtmlRawImage[] _panelRaws;
        ...
        void Awake()
        {
            _load = FindAnyObjectByType<LiteHtmlBenchmarkLoad>();
            _panels = GetComponentsInChildren<LiteHtmlView>(true);

            _panelRects = new RectTransform[_panels.Length];
            _panelSizes = new Vector2[_panels.Length];
            _panelRaws = new LiteHtmlRawImage[_panels.Length];

            for (int i = 0; i < _panels.Length; i++)
            {
                _panelRaws[i] = _panels[i].GetComponent<LiteHtmlRawImage>();
            ...
            _reportPath = Path.Combine(Application.persistentDataPath, "litehtml-bench.csv");
        }
  ...
  .v { color:#9fb4e8; }
        ...
        void ResetCacheCounters(int panels)
        {
            for (int p = 0; p < panels; p++)
            {
                _panels[p].QuadCacheStat(LiteHtmlNative.QuadCacheStat.Reset);
            }
        }
        ...
        Result Summarise(Scenario scenario, long heapBytes, int gen0)
        {
            ...
            for (int p = 0; p < scenario.Panels; p++)
            {
                r.CacheFramesFast += _panels[p].QuadCacheStat(LiteHtmlNative.QuadCacheStat.FramesFast);
                r.CacheFramesPartial += _panels[p].QuadCacheStat(LiteHtmlNative.QuadCacheStat.FramesPartial);
                r.CacheFramesRebuild += _panels[p].QuadCacheStat(LiteHtmlNative.QuadCacheStat.FramesRebuild);
                r.CacheQuadsReplayed += _panels[p].QuadCacheStat(LiteHtmlNative.QuadCacheStat.QuadsReplayed);
                r.CacheQuadsEmitted += _panels[p].QuadCacheStat(LiteHtmlNative.QuadCacheStat.QuadsEmitted);
            }
        ...
        void ShowReport()
        {
            ...
            go.GetComponent<LiteHtmlView>().LoadHtml(ReportPage());
        }
