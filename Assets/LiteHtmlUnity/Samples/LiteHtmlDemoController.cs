...
namespace LiteHtmlUnity.Samples
{
    ...
    /// times a second, and <see cref="LiteHtmlView.TotalMs"/> shows what it
    /// costs.
    /// </remarks>
    [RequireComponent(typeof(LiteHtmlView))]
    [AddComponentMenu("LiteHtml/Samples/LiteHtml Demo Controller")]
    public class LiteHtmlDemoController : MonoBehaviour
    {
        public enum Page
        {
            Overview,
        ...
        private LiteHtmlResources _resources;
        ...
        private void OnDemoButton(LiteHtmlElementClick click)
        {
            _buttonClicks++;
            _lastButton = string.IsNullOrEmpty(click.Action) ? click.Id : click.Action;
            _nextRefresh = 0f;

            Debug.Log($"[LiteHtml] butona basildi: id='{click.Id}' action='{click.Action}' " +
  ...
  .btn:active { background:#1b2547; }
        ...
        /// LiteHtmlView.BindClick, plus a data-action the handler reads. Pressing
        ...
        private void BuildIcons()
        {
            ...
            _resources.Register("coin", Icon(64, Coin));
            _resources.Register("gem", Icon(64, Gem));
            _resources.Register("potion", Icon(64, Potion));
        ...
        /// </summary>
        private Texture2D Icon(int size, Func<Vector2, Color> shade)
        {
        ...
        private static float Cover(float distance, float feather = 0.04f) =>
            Mathf.SmoothStep(1f, 0f, distance / feather);

        private static Color Coin(Vector2 p)
        {
            float r = p.magnitude;
        ...
        private static Color Gem(Vector2 p)
        {
        ...
        private static Color Potion(Vector2 p)
        {
